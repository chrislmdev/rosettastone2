/**
 * FinOps FOCUS 1.3 category from CSV label and/or free text.
 * Keep inferFocusCategoryFromText regex chain in sync with src/data/focusInference.js.
 */

/** Substring map for explicit CSV category / service_category fields. */
export const FOCUS_CATEGORY_MAP = {
  compute: "Compute",
  storage: "Storage",
  database: "Database",
...
/**
 * Infer FOCUS category from title + short name + description (CSV category empty / Other).
 */
export function inferFocusCategoryFromText(text) {
  const t = String(text || "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
...
export function resolvePricingFocusCategory({ category, service_category, title, shortname, description }) {
...
export function resolveParentFocusCategory({ category, csoparentservice, csoshortname }) {
